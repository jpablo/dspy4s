...
in `OrderedTensorOpsSuite`. The reusable hierarchy now distinguishes `MonoidalCategory`,
...
- Algebra 2: specified (grilled). Operation + law set, per-module reduction recipes, and sequencing in
  ...
  - **Previously implicit functors and actions are explicit:** `Functor` now includes its object mapping, allowing the
    ...
    Module input/output adaptation is `ModuleProfunctor`; total local-function lifting is `LiftFunctor`; and fallible
    lifting is `LiftEitherFunctor` from the generic Kleisli category generated by the `Either` `ScalaMonad`.
    `Mode` acts on modules through `MonoidAction`, whose corresponding functor has the opposite delooping as its
    source so the direction of a left action is explicit. Optimizer inspection factors as `InspectFunctor` followed by
    `ForgetMetadataFunctor`, with `ReadFunctor` retaining the arity-erased composite parameter view; the
