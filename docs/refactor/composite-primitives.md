...
stay introspectable/replaceable by `OptimizableTraversal` (optimizer-addressable). kyo-ai's `Thought` has no
...
capability can change what the optimizer tunes, so the binder must interplay with the `OptimizableTraversal`
...
- **Must add beyond kyo-ai: optimizer-addressability.** Every borrowed abstraction (Thought-augmentation,
  mode-middleware, enablement-binder) must remain introspectable/replaceable by `OptimizableTraversal`. kyo-ai has
