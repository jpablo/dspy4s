...
capability can change what the optimizer tunes, so the binder must interplay with the `OptimizableStructure`
...
- **Must add beyond kyo-ai: optimizer-addressability.** Every borrowed abstraction (Thought-augmentation,
  mode-middleware, enablement-binder) must remain introspectable/replaceable by `OptimizableStructure`. kyo-ai has
