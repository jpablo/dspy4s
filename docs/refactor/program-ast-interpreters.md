...
replacement—are implemented separately through per-node `Predictors` instances.
...
| Predictor traversal and replacement | A `Predictors` instance for every node |
| Record-boundary decoding | The existential package in `para.Program` |
| Optimizer state persistence | A traversal through `Predictors` |
...
places. Binary nodes also repeat state-vector slicing, reconstruction, and structural naming. `PairPredictors` and
`UnaryPredictors` reduce that duplication but do not remove the architectural coupling.
...
- Define execution semantics in one interpreter, including `ProgramCall` control propagation and `Prediction.raw`
  behavior.
- Derive one lawful `Predictors[Program[I, O]]` implementation from the tree instead of one instance per structural
  node.
...
flowchart LR
    ...
    Node --> Params["Predictor interpreter"]
...
sealed trait Atom[I, O]:
  type Repr <: Module[ProgramCall[I], Prediction[O]]
  val value: Repr
  val predictors: Predictors[Repr]
```

`Atom.of(module)` requires `Predictors[module.type-or-concrete-type]`. A genuinely parameter-free custom module must
continue to opt in explicitly with `Predictors.empty`; missing evidence must remain a compile error. This retains the
...
`Predictors` evidence would move down into `Atom`; its decoder would remain at the `RecordProgram` boundary.
...
### Predictor inspection and replacement

One tree interpreter replaces the structural `Predictors` instances:

- Parameter-free nodes contribute no views.
- Unary nodes recurse into their child.
- Binary nodes concatenate left then right.
- Leaves delegate to their packaged `Predictors[Repr]`.
...
```scala
given [I, O]: Predictors[Program[I, O]] = ProgramPredictorInterpreter.instance
```
...
That behavior should remain the default and becomes simpler through `Predictors[Program]`.
...
- Predictor traversal must be a homomorphism from structural composition into parameter-vector concatenation.
...
per-node `Predictors` boilerplate. It should be preferred if execution and predictor traversal remain the only important
interpretations.
...
- Provide `Predictors[Program]` and `ProgramRunner[RecordProgram]`.
...
- Remove self-interpreting structural case classes and their hand-written `Predictors` instances once parity is proven.
