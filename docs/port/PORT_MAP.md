...
| `code_act.py` | `CodeAct.scala` | ✅ ported. Iteration loop + fenced-code extraction + extractor; **tools-inside-code now works** on the sandboxed `DenoPyodideInterpreter` via `CodeAct.sandboxTools` (bridges `ToolFunction`s so generated Python calls them by name; G-20 part 1). On the plain subprocess interpreter, pre-load tools or go without. |
...
| `rlm.py` | — | Deferred (PORT_GAPS G-20) — the Deno+Pyodide interpreter it needs is now BUILT (`DenoPyodideInterpreter`: stateful REPL, variable injection, host tools, SUBMIT); only the RLM driver module remains, and upstream marks it `@experimental`. |
