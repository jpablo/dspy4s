...
| `parameter.py` | — | **Not ported.** Python's `Parameter` / `named_parameters` introspection enables optimizers to walk a program's mutable state generically. dspy4s uses the `PredictorTraversal[P]` / `PredictorLens[P]` typeclass pair instead: Scala 3 Mirror derivation enumerates `PredictorView`s, separates read-only `PredictorMetadata` from writable `PredictorState` (instructions/demos/config), and immutably replaces only state. An earlier `Parameter` + `BaseModule` + `ModuleGraphWalker` port shipped in Phase 1 and was removed; the interim `PredictOps` typeclass was itself superseded and removed in G-1 P6 (commit `1657f9c`), leaving `PredictorTraversal` as the sole introspection typeclass. |
...
- All of the above build on the **G-1** enablers (`PredictorTraversal`/`PredictorLens` introspection — relocated to
...
- **Program `save`/`load` + `dumpState`/`loadState`.** ✅ ported (commit `9c5a6db`) via `ProgramPersistence`
  (`modules/optimize/.../ProgramPersistence.scala`), built on `PredictorTraversal`. JSON state of
  `{ "predictors": { "predictor-0": <PredictorState>, ... } }`; ordinal IDs are validated on load.
...
- **Per-module `config` + bound LM.** ✅ ported — `DynamicPredict`, `Predict[I, O]`, and `ChainOfThought[I, O]`
  carry immutable module-level `config` (commit `b85fe27`, merged under per-call config). `DynamicPredict` and
  `Predict` can also carry an optional bound LM (`Predict.withLm` / `Predict.boundLm`, commit `b2d0096`; a bound
  LM wins over the ambient `RuntimeContext` LM).
  See PORT_GAPS G-3.
