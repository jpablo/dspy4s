...
|---|---|---|---|---|
| **ChatAdapter framing** | Uses `[[ ## field ## ]]` start markers and `[[ ## completed ## ]]` end marker. State machine handles missing end markers by buffer flush. | Uses `prefix: value` line framing (one field per line). No completion marker. | Simpler, matches dspy4s's typed `Signature.outputFields` ordering directly. | `ChatAdapter.scala`, [STREAMING_POSTPONED.md v1.2 notes](STREAMING_POSTPONED.md) |
| **Streaming: `tokens.last` index** | Python streaming tests use `all_chunks[-2]` to skip a trailing completion-marker flush. | Tests assert against `tokens.last`. | Consequence of the no-completion-marker delta above. | `StreamingLiveSuite` test comments |
