...
class Phase5SpecMacroSuite extends FunSuite:
  ...
  test("compile error: method not wrapped in InputField/OutputField") {
    val errors = compileErrors("""
      trait BadSpec extends dspy4s.signatures.Spec:
        def sentence: String   // wrong: not wrapped in InputField/OutputField
      val sig = dspy4s.signatures.Signature.of[BadSpec]
    """)
    assert(
  ...
  test("compile error: method with parameters") {
    val errors = compileErrors("""
      trait BadSpec extends dspy4s.signatures.Spec:
        def f(x: Int): dspy4s.signatures.InputField[String]
      val sig = dspy4s.signatures.Signature.of[BadSpec]
    """)
    assert(
  ...
  test("compile error: missing Schema for inner type") {
    val errors = compileErrors("""
      class NotDecodable
      trait BadSpec extends dspy4s.signatures.Spec:
        def field: dspy4s.signatures.OutputField[NotDecodable]
      val sig = dspy4s.signatures.Signature.of[BadSpec]
    """)
    assert(
  ...
  test("compile error: empty spec trait") {
    val errors = compileErrors("""
      trait EmptySpec extends dspy4s.signatures.Spec
      val sig = dspy4s.signatures.Signature.of[EmptySpec]
    """)
    assert(
  ...
  test("compile error: concrete (non-abstract) methods on a spec trait") {
    val errors = compileErrors("""
      trait BadSpec extends dspy4s.signatures.Spec:
        def question: dspy4s.signatures.InputField[String]
