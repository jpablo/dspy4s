...
class Phase7FunctionMacroSuite extends FunSuite:
  ...
  test("compile error: missing input Schema") {
    val errors = compileErrors("""
      ...
      val sig = dspy4s.signatures.Signature.from(badInput)
    """)
    assert(
  ...
  test("compile error: Unit output") {
    val errors = compileErrors("""
      ...
      val sig = dspy4s.signatures.Signature.from(noOutput)
    """)
    assert(
  ...
  test("compile error: fromType Unit output") {
    val errors = compileErrors("""
      val sig = dspy4s.signatures.Signature.fromType[String => Unit]("Bad")
    """)
    assert(
      errors.contains("not Unit"),
      s"expected helpful error about Unit output, got:\n$errors"
