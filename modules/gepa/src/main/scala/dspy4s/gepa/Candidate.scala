...
object Candidate:
  ...
  def seed[P](program: P)(using ps: OptimizableTraversal[P]): Candidate =
  ...
  /** Human-readable view for diagnostics and UI. A vector is used because structural display names are not the unique
    * identity key and may collide in unusual user-defined `OptimizableTraversal` instances.
    */
  def named[P](program: P, candidate: Candidate)(using ps: OptimizableTraversal[P]): Vector[(String, Option[String])] =
