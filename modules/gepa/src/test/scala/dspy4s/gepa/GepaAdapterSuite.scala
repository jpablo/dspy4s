...
final class GepaAdapterSuite extends FunSuite:
  ...
  private def countingBackend(calls: AtomicInteger): PredictionBackend = new PredictionBackend:
  ...
  test("adapter evaluates a candidate and captures parameter-tagged evidence") {
    ...
    val result  = run(adapter.evaluate(Vector(example), Candidate.seed(program), captureEvents = true), countingBackend(calls))
