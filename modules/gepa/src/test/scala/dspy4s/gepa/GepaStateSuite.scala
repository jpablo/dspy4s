...
class GepaStateSuite extends FunSuite:
  ...
  private def state(subscores: Vector[Double]*): GepaState =
    GepaState(
      candidates = CandidatePool.applyUnsafe(
        subscores.indices.map(i => Map(ParameterId("p0") -> Some(s"instr$i"))).toVector
      ),
  ...
  test("GepaState rejects ragged valSubscores up front (instead of an IndexOutOfBounds in paretoFrontier)") {
    intercept[IllegalArgumentException] {
      val _ = GepaState(
        candidates = CandidatePool.applyUnsafe(
          Vector(Map(ParameterId("p0") -> Some("a")), Map(ParameterId("p0") -> Some("b")))
        ),
  ...
  test("add appends a candidate and accrues metric calls") {
    val s0 = GepaState.seed(Map(ParameterId("p0") -> Some("seed")), Vector(0.5, 0.5), metricCalls = 2)
    val s1 = s0.add(Map(ParameterId("p0") -> Some("child")), Vector(1.0, 1.0), parents = Vector(0), metricCalls = 2)
  ...
  test("ancestors walks the full lineage, including a merge's two branches up to a common ancestor") {
    ...
    val s = GepaState(
      candidates = CandidatePool.applyUnsafe(Vector.tabulate(4)(i => Map(ParameterId("p0") -> Some(i.toString)))),
  ...
  test("RoundRobin picks one component per call and cycles through, wrapping the pointer") {
    val cs = Vector(ParameterId("p0"), ParameterId("p1"), ParameterId("p2"))
    // Thread the pointer through six calls; expect a, b, c, a, b, c.
    val (picks, _) = (0 until 6).foldLeft((Vector.empty[ParameterId], 0)) { case ((acc, ptr), _) =>
      val (chosen, next) = ComponentSelector.RoundRobin.select(cs, ptr)
      (acc :+ chosen.head, next)
    }
    assertEquals(
      picks,
      Vector(0, 1, 2, 0, 1, 2).map(i => ParameterId(s"p$i"))
    )
  }
  ...
  test("RoundRobin normalizes an out-of-range pointer") {
    ...
    assertEquals(
      ComponentSelector.RoundRobin.select(Vector(ParameterId("p0"), ParameterId("p1")), 5),
      (Vector(ParameterId("p1")), 0)
    )
  }
