/** Talk to Your Data: offline self-check (no LM, no Deno).
  *
  * Proves the dataset + Scala query engine are internally consistent before any model is involved: the parts that the
  * gold answer key and the GEPA metric depend on. Run it any time to sanity-check the foundation.
  *
...
object SelfCheck:
  ...
  private val sampleQp = QueryPlan(
    ...
    Some(Sort("value", descending = true)),
    Some(3),
    AnswerKind.Category
  )

  private val sampleAr =
    AnalysisResult("Electronics", Some(123.45), List("assumed full-year data"), "summed totals by category")
  ...
  def checks(): Vector[(String, Boolean)] =
    ...
    val arShape      = Agent.executorOutputShape
    ...
    val byCategory   = categories.map(c =>
      value(QueryPlan(
        ...
        Some("total"),
        Nil,
        List(Filter("category", FilterOp.Eq, c)),
        None,
        None,
        None,
        AnswerKind.Number
    ...
    val byRegionCnt = regions.map(r =>
      value(QueryPlan(
        Agg.Count,
        None,
        Nil,
        List(Filter("region", FilterOp.Eq, r)),
        None,
        None,
        None,
        AnswerKind.Number
    ...
    Vector(
      ...
      f"revenue by category sums to total ($byCategory%.2f vs $totalRevenue%.2f)" ->
        (math.abs(byCategory - totalRevenue) < 0.5),
      s"order count by region sums to total (${byRegionCnt.toInt} vs ${totalCount.toInt})" ->
        (byRegionCnt.toInt == totalCount.toInt),
      f"average == total / count ($avg%.4f vs ${totalRevenue / totalCount}%.4f)" ->
        (math.abs(avg - totalRevenue / totalCount) < 0.01),
