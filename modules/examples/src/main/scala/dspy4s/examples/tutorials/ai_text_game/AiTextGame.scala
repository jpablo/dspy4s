/** Building a Creative Text-Based AI Game with DSPy
  *
  * Source: docs/docs/tutorials/ai_text_game/index.md Upstream:
  * https://github.com/stanfordnlp/dspy/blob/main/docs/docs/tutorials/ai_text_game/index.md Status: translated (the DSPy
  * core: the three signatures + the GameAI module composing them, from snippets 1/2). The rest of the tutorial is plain
  * game plumbing around DSPy and is out of scope: the `Player` / `GameContext` save/load JSON (snippet 1), and the
  * console rendering, menus, character creation, and `input()` game loop (snippets 3/4). Minimal `Player` /
...
/** Minimal carriers for the game state Python keeps in `Player` / `GameContext` dataclasses — enough to build the
  * formatted-string inputs the signatures take. (The save/load JSON is out of scope.)
  */
...
object AiTextGame:
  ...
  final class GameAI:
    ...
    def handleDialogue(npcName: String, playerInput: String, context: GameContext)(using
        RuntimeContext
    )
    ...
    def resolveAction(action: String, player: Player, context: GameContext)(using
        RuntimeContext
    )
