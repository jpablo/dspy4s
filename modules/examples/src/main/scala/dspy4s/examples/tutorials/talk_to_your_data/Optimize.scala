/** Talk to Your Data: GEPA optimization of the planner.
  ...
  * construction). On a miss, the feedback hands GEPA the *correct* plan to reflect on. That clean signal is what makes
  * the before/after lift real and reproducible.
  */
...
object Optimize:
  ...
  def planner(instructions: String): DynamicPredict =
    DynamicPredict(
      layout = Agent.plannerSignature(instructions).layout,
      config = DynamicValues.record("temperature" := 0.0)
    )
