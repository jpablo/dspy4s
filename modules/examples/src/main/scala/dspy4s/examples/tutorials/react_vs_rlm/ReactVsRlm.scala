...
import dspy4s.core.contracts.{DspyError, DynamicValues, SandboxTool, TypeRef}
import dspy4s.core.runtime.DenoPyodideInterpreter
import dspy4s.examples.Demo
import dspy4s.programs.*
import dspy4s.programs.contracts.{Prediction, Tool, ToolCallRequest}
...
object DrugSafetyTools:
  ...
  final case class Built(host: Vector[Tool], sandbox: Vector[SandboxTool])
  ...
  def build(recorder: CallRecorder): Built =
    val definitions = Vector(
      Definition(
        ...
        Vector("drug_a" -> TypeRef.string, "drug_b" -> TypeRef.string),
        args =>
      ...
      Definition(
        ...
        Vector("drug" -> TypeRef.string, "condition" -> TypeRef.string),
        args =>
      ...
      Definition(
        ...
        Vector("drug" -> TypeRef.string),
        args =>
          val d = arg(args, "drug")
          DrugSafetyData.drugClassDb.getOrElse(DrugSafetyData.norm(d), s"unknown class for $d")
      )
    )
    Built(definitions.map(_.host), definitions.map(_.sandbox))
...
object ReactVsRlm:
  ...
  val signature = Signature.derived[MedicationInput, RiskReport]("DrugSafety", taskInstructions)
  ...
  private val reactGenerator = Program
    ...
    .contramap[ReAct.StepInput[MedicationInput]](input =>
      ReactDecisionPrompt(
        input.input.medications,
        input.input.conditions,
        input.trajectory.mkString("\n"),
        availableTools
      )
    )
    .map { decision =>
      if decision.finish then ReAct.Step(decision.thought, ReAct.Action.Finish())
        ...
        val entries = decision.toolName match
          case "check_drug_interaction" => Vector(
              "drug_a" -> DynamicValues.fromAny(decision.drugA.getOrElse("")),
              "drug_b" -> DynamicValues.fromAny(decision.drugB.getOrElse(""))
            )
          case "check_contraindication" => Vector(
              "drug"      -> DynamicValues.fromAny(decision.drug.getOrElse("")),
              "condition" -> DynamicValues.fromAny(decision.condition.getOrElse(""))
            )
          case _ => Vector("drug" -> DynamicValues.fromAny(decision.drug.getOrElse("")))
        ReAct.Step(
          decision.thought,
          ReAct.Action.Invoke(ToolCallRequest(decision.toolName, DynamicValues.recordFromEntries(entries)))
        )
    }
  ...
  private val reactExtractor = Program
    ...
    .contramap[ReAct.ExtractInput[MedicationInput]](input =>
      ExtractPrompt(input.input.medications, input.input.conditions, input.trajectory.mkString("\n"))
    )
  ...
  private val rlmGenerator = Program
    .predict(
      Signature.derived[RlmDecisionPrompt, RlmDecision](
        "DrugSafetyRlmStep",
        taskInstructions +
          "\nWrite Python that uses the available host functions. End with SUBMIT(riskReport=...)."
      )
    )
    .contramap[RLM.ActionInput[MedicationInput]](input =>
      RlmDecisionPrompt(
        input.input.medications,
        input.input.conditions,
        input.history.mkString("\n"),
        input.iteration,
        input.maxIterations,
        availableTools
      )
    )
    .map(value => RLM.ActionStep(value.reasoning, value.code))
  ...
  private val rlmExtractor = Program
    ...
    .contramap[RLM.ExtractInput[MedicationInput]](input =>
      ExtractPrompt(input.input.medications, input.input.conditions, input.history.mkString("\n"))
    )
  ...
  def runReAct(medications: String, conditions: String)(using predictionBackend: PredictionBackend): RunResult =
    val recorder = new CallRecorder
    val tools    = DrugSafetyTools.build(recorder)
    // --8<-- [start:react-agent]
  ...
  def runRlm(medications: String, conditions: String)(using predictionBackend: PredictionBackend): RunResult =
    ...
    val agent = RLM(
      rlmGenerator,
      RLM.replExecutor(Shape.derived[MedicationInput], Shape.derived[RiskReport]),
      rlmExtractor,
      maxIterations = 12
    )
    // --8<-- [end:rlm-agent]
