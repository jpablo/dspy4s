...
object Cache:
  ...
  /** Build a run-local backend from `lm` and answer one question. */
  def ask(lm: LanguageModel, question: String)(using RuntimeContext): Either[DspyError, String] =
    val context = summon[RuntimeContext].copy(lm = Some(lm), adapter = Some(ChatAdapter()))
    val backend = new LivePredictionBackend(lm, ChatAdapter(), context)
    Demo.run(qa, (question = question))(using backend).map(_.output.answer)
  ...
  def usageAcrossCachedCalls(lm: LanguageModel, question: String)(using
    ...
    UsageTracking.withNewTracker { tracker =>
      for
        _ <- ask(managed, question) // miss: records usage
        _ <- ask(managed, question) // hit: fast, no new usage
