/** Automated Code Generation from Documentation with DSPy
  *
  * Source: docs/docs/tutorials/sample_code_generation/index.md Upstream:
  * https://github.com/stanfordnlp/dspy/blob/main/docs/docs/tutorials/sample_code_generation/index.md Status: translated
  * (the DSPy core: the two signatures, the refine CoT, and the composing agent, from snippets 1/2/3/4). Everything else
  * in the tutorial is plain I/O around DSPy and is out of scope: the `DocumentationFetcher` (requests + BeautifulSoup +
  * html2text), the interactive `input()` console session, and JSON file saving. `learnFromDocs` therefore takes the
...
object SampleCodeGeneration:
  ...
  final class DocumentationLearningAgent:
    ...
    def learnFromDocs(libraryName: String, combinedContent: String)(using
        PredictionBackend
    )
        : Either[DspyError, LibraryInfo] =
      Demo.run(analyzeDocs, (library_name = libraryName, documentation_content = combinedContent)).map { analysis =>
        LibraryInfo(
          library = libraryName,
    ...
    def generateExample(info: LibraryInfo, useCase: String, requirements: String = "")(using
        PredictionBackend
    )
      ...
      Demo.run(generateCode, (library_info = infoText, use_case = useCase, requirements = requirements)).map { r =>
        GeneratedExample(
    ...
    def refine(code: String, feedback: String)(using PredictionBackend): Either[DspyError, (String, List[String])] =
      Demo.run(refineCode, (code = code, feedback = feedback))
  ...
  )(using PredictionBackend): Either[DspyError, (LibraryInfo, Vector[GeneratedExample])] =
