/** Talk to Your Data: the (synthetic, deterministic) dataset and the gold question set.
  *
  * A seeded generator produces ~10k e-commerce orders, so the data (and therefore every gold answer) is reproducible
  * and license-clean to ship. The agent only ever sees the data as CSV text (too big to put in a prompt, which is
  * exactly why the RLM stage writes code to crunch it). The gold answers are computed by the Scala [[QueryEngine]] from
  * a hand-written plan per question, so the answer key is correct *by construction* (no LLM judge). That is the
  * foundation that makes the GEPA optimization measurable.
  */
...
object Dataset:
  ...
  private val catalog: Map[String, Vector[(String, Double)]] = Map(
    "Electronics" ->
      Vector("Headphones" -> 79.99, "Laptop" -> 1299.00, "Phone" -> 899.00, "Charger" -> 19.99, "Monitor" -> 249.00),
    "Apparel" -> Vector("T-Shirt" -> 14.99, "Jeans" -> 49.99, "Jacket" -> 89.99, "Sneakers" -> 64.99, "Socks" -> 7.99),
    "Home"    ->
      Vector("Lamp" -> 34.99, "Cookware Set" -> 119.00, "Towels" -> 24.99, "Vacuum" -> 199.00, "Pillow" -> 29.99),
    "Grocery" ->
      Vector("Coffee" -> 12.99, "Olive Oil" -> 9.99, "Cereal" -> 4.49, "Cheese" -> 7.49, "Chocolate" -> 3.99),
    "Toys" ->
      Vector("Building Blocks" -> 39.99, "Board Game" -> 24.99, "Doll" -> 19.99, "RC Car" -> 59.99, "Puzzle" -> 12.99),
  ...
  def generate(n: Int, seed: Long): Vector[Order] =
    ...
    Vector.tabulate(n) { i =>
      ...
      Order(
        ...
        unitPrice,
        discount,
        roundCents(quantity * unitPrice * (1 - discount))
      )
    }
  ...
  val goldset: Vector[GoldQuestion] = Vector(
    ...
    GoldQuestion(
      ...
      num(Agg.Sum, Some("total"), List(f("region", FilterOp.Eq, "West")))
    ),
    GoldQuestion(
      "Which category has the highest total revenue?",
      topGroup(Agg.Sum, "category", AnswerKind.Category, Some("total"))
    ),
    ...
    GoldQuestion(
      ...
      num(Agg.Sum, Some("total"), List(f("category", FilterOp.Eq, "Electronics")))
    ),
    GoldQuestion(
      "How many orders had a quantity greater than 3?",
      num(Agg.Count, None, List(f("quantity", FilterOp.Gt, "3")))
    ),
    ...
    GoldQuestion(
      "What is the total quantity sold of Books?",
      num(Agg.Sum, Some("quantity"), List(f("category", FilterOp.Eq, "Books")))
    ),
    GoldQuestion(
      "Which product category has the lowest total revenue?",
      topGroup(Agg.Sum, "category", AnswerKind.Category, Some("total"), descending = false)
    ),
    GoldQuestion("What is the total revenue in Q1 (January to March 2024)?", num(Agg.Sum, Some("total"), Nil, q1)),
    GoldQuestion(
      "How many North-region orders had a discount of at least 0.1?",
      num(Agg.Count, None, List(f("region", FilterOp.Eq, "North"), f("discount", FilterOp.Gte, "0.1")))
    ),
    GoldQuestion(
      "What is the average order total for Grocery?",
      num(Agg.Average, Some("total"), List(f("category", FilterOp.Eq, "Grocery")))
    ),
    GoldQuestion(
      "Which region generated the highest revenue?",
      topGroup(Agg.Sum, "region", AnswerKind.Category, Some("total"))
    ),
    GoldQuestion(
      "What is the total revenue from orders over $200?",
      num(Agg.Sum, Some("total"), List(f("total", FilterOp.Gt, "200")))
    ),
    GoldQuestion("What is the median order total?", num(Agg.Median, Some("total"))),
    GoldQuestion(
      "What is the total revenue for Apparel in the South region?",
      num(Agg.Sum, Some("total"), List(f("category", FilterOp.Eq, "Apparel"), f("region", FilterOp.Eq, "South")))
    ),
    GoldQuestion(
      "How many orders were placed in December 2024?",
      num(Agg.Count, None, Nil, Some(TimeRange("date", Some("2024-12-01"), Some("2024-12-31"))))
    ),
    GoldQuestion(
      "What is the maximum order total in the Toys category?",
      num(Agg.Max, Some("total"), List(f("category", FilterOp.Eq, "Toys")))
    ),
    GoldQuestion(
      "Which category has the highest average order value?",
      topGroup(Agg.Average, "category", AnswerKind.Category, Some("total"))
    ),
    GoldQuestion(
      "What is the total revenue for Home in Q4?",
      num(Agg.Sum, Some("total"), List(f("category", FilterOp.Eq, "Home")), q4)
    ),
