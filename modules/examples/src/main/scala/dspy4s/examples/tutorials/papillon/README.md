...
1. A multi-stage program where a small local LM uses an external tool.
...
- Multi-stage programs: compose `ProgramWithEnv[I, O, R]` values (the [email extraction](../email_extraction/EmailExtraction.scala)
...
- A judge-as-metric: implement the effectful `Metric` trait and run a judge child program through its explicit
  `PredictionBackend`; see [`learn/evaluation`](../../learn/evaluation/README.md).

Teacher-led few-shot optimization is supported by `BootstrapFewShot` and `MIPROv2`. Weight finetuning is not ported.
