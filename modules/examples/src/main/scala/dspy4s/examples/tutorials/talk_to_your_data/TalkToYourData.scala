 ...
 *                        lists, not a string. The same types constrain the LM, decode its reply, drive the
 *                        optimizer's metric, and render the report. Change a field and the compiler finds every
 *                        site that breaks. (See Schemas.scala.)
 *   2. RLM               The dataset (10k rows of CSV, too big to prompt) is handed to a sandboxed Python REPL;
 *                        the model writes code to compute the answer. The LM never does the arithmetic — the
 *                        sandbox does — and the JVM independently re-computes the same plan and must agree.
 *   3. GEPA              The planner's instruction is evolved against a grounded metric (the Scala engine vs a
 *                        by-construction gold answer key), turning a vague baseline into a reliable planner.
 *
 * Two parts run here:
 *   - GEPA optimization of the planner — needs only an LM (the metric is the deterministic Scala engine).
 *   - The full plan->act->verify->refine agent on a few questions — needs Deno for the RLM sandbox.
 *
 ...
 * Offline foundation check (no LM/Deno): ...talk_to_your_data.tytdSelfCheckMain
 */
    ...
    case Right(lm) =>
      RuntimeEnvironment.withSettings(RuntimeContext(lm = Some(lm), adapter = Some(JSONAdapter()))) {
        ...
        if denoAvailable then
          ...
          println("   (The GEPA optimization above ran without it — its metric is the Scala engine, not the sandbox.)")
      }
