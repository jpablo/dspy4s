...
private case class EnrichedEmail(
    ...
    financialAmount: Option[Double],
    importantDates : List[String],
    contactInfo    : List[String]
)
private case class SummarizedEmail(enriched: EnrichedEmail, summary: String)
...
object EmailExtraction:
  ...
  object EmailProcessor:
    ...
    ).map { case (value, result) =>
      EnrichedEmail(
        value,
        result.key_entities,
        result.financial_amount,
        result.important_dates,
        result.contact_info
      )
    }

    private val summarized = enriched >>> (
      Program.identity[EnrichedEmail] &&& summarizer.contramap[EnrichedEmail](value =>
        (
          email_subject = value.classified.request.subject,
          email_body = value.classified.request.body,
          key_entities = value.entities
        )
      )
    ).map { case (value, result) => SummarizedEmail(value, result.summary) }

    val program = summarized >>> (
      Program.identity[SummarizedEmail] &&& actionGenerator.contramap[SummarizedEmail](value =>
        (
          email_type = value.enriched.classified.emailType,
          urgency = value.enriched.classified.urgency,
          email_summary = value.summary,
          extracted_entities = value.enriched.entities
        )
      )
    ).map { case (value, actions) =>
      val enriched   = value.enriched
      val classified = enriched.classified
      EmailAnalysis(
        classified.emailType,
        classified.urgency,
        value.summary,
        enriched.entities,
        enriched.financialAmount,
        enriched.importantDates,
    ...
    def run(request: EmailRequest)(using PredictionBackend): Either[DspyError, EmailAnalysis] =
      Demo.run(program, request).map(_.output)
  // --8<-- [end:processor]
  ...
  def describe(email: SampleEmail)(using PredictionBackend): String =
    EmailProcessor.run(EmailRequest(email.subject, email.body, email.sender)) match
