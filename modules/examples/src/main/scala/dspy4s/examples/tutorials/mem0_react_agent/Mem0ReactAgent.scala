/** A memory-enabled ReAct agent with a replaceable memory boundary.
  *
  * The upstream tutorial uses Mem0. dspy4s has no Mem0 client, so this port defines the small interface that an adapter
  * must implement and includes an in-memory implementation. The ReAct and tool code is unchanged when a real Mem0
  * adapter replaces it.
  */
...
object Mem0ReactAgent:
  ...
  def tools(store: MemoryStore): Vector[Tool] =
    Vector(
      Tool(
        "store_memory",
