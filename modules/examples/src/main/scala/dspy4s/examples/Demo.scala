...
/** Shared runner for the example `@main`s: initialize an OpenAI LM from the environment, install it (plus a
  * `ChatAdapter`) as the active [[RuntimeContext]], and run `body` with that context as the ambient `given`. The model
  * defaults to `$DSPY_MODEL` (or `gpt-5.5`); `OPENAI_API_KEY` must be set.
  *
