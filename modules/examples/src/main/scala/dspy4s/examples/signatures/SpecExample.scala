...
object SpecExample:
  ...
  /** A multi-field QA spec. Field order, names, types, normalized prefixes, and enum metadata all flow from the trait
    * declarations.
    */
  val qa = Signature.of[QASpec]

  /** A structured-output spec. Nested products, lists, and primitive coercions are decoded through zio-blocks-schema.
    */
