...
object LanguageModels:
  ...
  // for a run means constructing and providing `LivePredictionBackend`. `Demo.withLm` does this for runnable examples.
  def configureDefault(model: String = "gpt-4o-mini"): Either[DspyError, PredictionBackend] =
    OpenAiLanguageModel.fromEnv(model).map { lm =>
      val context = RuntimeContext(lm = Some(lm), adapter = Some(ChatAdapter()))
      new LivePredictionBackend(lm, ChatAdapter(), context)
    }
  ...
  def ask(question: String)(using PredictionBackend): Either[DspyError, String] =
    Demo.run(qa, (question = question)).map(_.output.answer)
  ...
  // A scoped model is another backend supplied only to the second run.
  ...
  // for cache-busting — in dspy4s `rollout_id` is a first-class field on `RunOptions`.
  // --8<-- [start:lm-config]
  ...
  // --8<-- [start:lm-errors]
  def askHandlingErrors(question: String)(using PredictionBackend): String =
    ask(question) match
