...
import dspy4s.programs.{PredictionBackend, PredictionRequest, ProgramEvent, RecordProgram}
...
object Metrics:
  ...
  val offlineBackend: PredictionBackend = new PredictionBackend:
    def generate(@annotation.unused request: PredictionRequest) =
      ZIO.fail(RuntimeError("offline_example", "This metric does not make prediction calls"))
  ...
  )(using PredictionBackend): Vector[Either[DspyError, Double]] =
    devset.map(x => program(x).flatMap(pred => Demo.runEffect(metric.score(x, pred, Vector.empty))))
  ...
  def evaluator[I, O](
    ...
    Evaluate(program, devset, metric, EvaluateOptions(parallelism = 1))
  // --8<-- [end:metric-evaluate]
  // Run the returned effect with the same explicit backend used by `ProgramRunner`.
...
@main def metricsMain(): Unit =
  ...
  println("validate_answer('Lyon'):  " + Demo.runEffect(Metrics.validateAnswer.score(ex, pred("Lyon"), Vector.empty)))
...
@main def metricsJudgeMain(): Unit =
  Demo.withLm {
    ...
    println("SemanticF1 score: " + Demo.runEffect(Metrics.semanticF1.score(gold, pred, Vector.empty[ProgramEvent])))
  }
