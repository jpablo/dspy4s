...
2. **Define a metric.** A metric scores a prediction against an example. In dspy4s a metric implements the
   `Metric` trait — `score(example, prediction, events): ZIO[PredictionBackend, DspyError, Double]` — and `FunctionMetric(name) { (ex, pred) => … }`
...
3. **Run development evaluations.** `Evaluate(program, devset, metric, EvaluateOptions(...))` runs your program over the
...
An effectful metric can call a judge program through its explicit `PredictionBackend`. Program journals provide
`ProgramEvent` values instead of a mutable ambient trace. There is no bundled retriever, but an injected retrieval
program can emit observed events.
