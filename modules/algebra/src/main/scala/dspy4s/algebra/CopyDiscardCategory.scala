...
trait MonoidalCategory[Hom[_, _]] extends TensorOps[Hom]:
  ...
  def associatorNaturality[A, B, C, D, E, F2](
      ...
      g: Hom[B, E],
      h: Hom[C, F2]
  ): IsEq[Hom[((A, B), C), (D, (E, F2))]] = (tensor(tensor(f, g), h) >>> associator[D, E, F2].forward) <->
...
trait SymmetricMonoidalCategory[Hom[_, _]] extends MonoidalCategory[Hom]:
  ...
  def braidingNaturality[A, B, C, D](
      f: Hom[A, C],
      g: Hom[B, D]
  ): IsEq[Hom[(A, B), (D, C)]] = (tensor(f, g) >>> braiding[C, D].forward) <->
...
trait CopyDiscardCategory[Hom[_, _]] extends SymmetricMonoidalCategory[Hom]:
  ...
  def copyLeftCounit[A]: IsEq[Hom[A, A]] = (copy[A] >>> tensor(discard[A], id[A]) >>> leftUnitor[A].forward) <-> id[A]

  @Law("discard is a right counit for copy")
  def copyRightCounit[A]: IsEq[Hom[A, A]] = (copy[A] >>> tensor(id[A], discard[A]) >>> rightUnitor[A].forward) <-> id[A]

  @Law("copy is cocommutative")
  def copyCocommutativity[A]: IsEq[Hom[A, (A, A)]] = (copy[A] >>> braiding[A, A].forward) <-> copy[A]
...
trait MarkovCategory[Hom[_, _]] extends CopyDiscardCategory[Hom]:
  ...
  def discardNaturality[A, B](f: Hom[A, B]): IsEq[Hom[A, Unit]] = (f >>> discard[B]) <-> discard[A]
