...
final class EvaluateSuite extends FunSuite:
  ...
  test("effectful evaluation runs record programs in parallel and preserves dataset order") {
    ...
    val result = run(Evaluate(program, dataset, exact, EvaluateOptions(parallelism = 2)))
  ...
  test("program failure becomes an explicit scored row") {
    ...
    val options = EvaluateOptions(failureScore = -1.0, includeErrors = true)
    val result  = Unsafe.unsafe { implicit unsafe =>
      Runtime.default.unsafe
        .run(Evaluate(program, dataset, exact, options).provideEnvironment(ZEnvironment(failingBackend)))
        .getOrThrowFiberFailure()
    }
