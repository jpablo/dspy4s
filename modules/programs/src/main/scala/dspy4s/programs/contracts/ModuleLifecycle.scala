...
/** Whether a module boundary is observable and, when it is, how its call and result project into runtime records.
...
object ModuleLifecycle:
  ...
  final case class Observed[I, O](observation: CallObservation[I, O]) extends ModuleLifecycle[I, O]

  def transparent[I, O]: ModuleLifecycle[I, O] = Transparent()

  def observed[I, O](
      inputs: I => DynamicValue.Record,
      traceEnabled: I => Boolean,
  ...
  val dynamic: ModuleLifecycle[ProgramCall[DynamicValue.Record], DynamicPrediction] =
  ...
  def typed[I, O](inputs: ProgramCall[I] => DynamicValue.Record): ModuleLifecycle[ProgramCall[I], Prediction[O]] =
    observed(inputs, _.traceEnabled, _.raw.values)

  /** Signature-backed typed observation. */
  def typed[I, O](shape: Shape[I]): ModuleLifecycle[ProgramCall[I], Prediction[O]] =
  ...
  def typedWithoutInputs[I, O]: ModuleLifecycle[ProgramCall[I], Prediction[O]] =
