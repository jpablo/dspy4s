...
final case class Parallel(
  ...
  private def resolvedExecutor(using RuntimeContext): ParallelExecutor =
    ...
    ParallelExecutor(
      numThreads = numThreads.getOrElse(settings.get(SettingKeys.numThreads).getOrElse(8)),
      maxErrors = maxErrors.getOrElse(settings.get(SettingKeys.maxErrors).getOrElse(10)),
      timeout = timeout
    )

  def run(
      tasks: Vector[(PredictProgram, ProgramCall)]
  )(using RuntimeContext): Either[DspyError, ParallelExecutionResult[Prediction]] =
    resolvedExecutor.executeEither[(PredictProgram, ProgramCall), Prediction](
