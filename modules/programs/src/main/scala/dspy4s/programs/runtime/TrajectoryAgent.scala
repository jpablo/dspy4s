...
/** The shared "gather a trajectory, then extract the answer from it" agent shape behind `ReAct` and `CodeAct` (the
  ...
  * renders, and the per-iteration policy/tool/code step — stays in the caller's `step` closure and `render` function.
  * The final `decodePrepended` into the typed `WithReasoning[O]` stays in the module (where the prepend evidence
  * lives).
  */
object TrajectoryAgent:
  ...
  /** Run the bounded loop then the extractor. Returns the extractor's raw prediction paired with the rendered
    ...
    *
    * @param baseCall
    *   the underlying program call (controls inherited from the typed call)
    * @param inputs
    *   the base task inputs the extractor sees alongside the trajectory
    * @param maxIterations
    *   loop budget; on exhaustion the accumulated trajectory is extracted as-is
    * @param trajectoryKey
    *   the input field name the rendered trajectory is written to ("trajectory")
    * @param render
    *   renders a trajectory to the prompt string (also used for the overflow-truncation retry)
    * @param extractor
    *   the final extractor predict
    * @param step
    *   one iteration: run the policy + action, append an entry, decide Continue / Done
    */
  def runAndExtract[S](
      baseCall: ProgramCall[DynamicValue.Record],
    ...
    for
      ...
      extracted  <- truncateOnOverflow(trajectory, extractAttempts)(render) { view =>
          ...
          input = inputs.updated(trajectoryKey, DynamicValue.Primitive(PrimitiveValue.String(view)))
                      ))
                    }._1
