...
/** Stable identity of a learnable predictor within a program's [[Predictors]] traversal.
  ...
  *
  * A [[PredictorId]] is deliberately not a display path. Human-readable paths come from [[Predictors.readNamed]] and
...
/** One focus of a [[Predictors]] traversal: stable machine identity, structural display name, and current value. */
final case class IdentifiedPredictor(id: PredictorId, displayName: String, predictor: DynamicPredict)
