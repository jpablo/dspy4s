...
object ProgramOfThought:
  ...
  def apply[I, O, RG, RR, RE, RA](
    ...
    val prepare = Program.liftEither[(State[I], GeneratedCode), Either[Decision, (State[I], String)]] {
      case (state, result) => result.code match
          case None => reject(
              state,
              "",
          ...
          case Some(rawCode) => parseCode(rawCode) match
    ...
    val execute = (
      ...
      case ((state, code), CodeExecutionResult.Failed(error)) => reject(state, code, error, maxAttempts)
    }
