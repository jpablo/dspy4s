...
/** The complete writable state of one optimizer-addressable predictor.
  ...
  * [[Predictor]] a lawful state lens and gives [[Predictors]] and Para one homogeneous parameter carrier.
  *
  * @param instructions
  ...
  def fromState(state: DynamicValue.Record): Either[DspyError, PredictorState] =
    def readInstructions: Either[DspyError, Option[String]] =
      DynamicValues.recordGet(state, "instructions") match
        ...
        case None    => Left(ValidationError("Predictor state is missing 'instructions'"))
        case Some(_) => Left(ValidationError("Predictor state 'instructions' must be a string or null"))
            ...
            (acc, raw) =>
              for
                ...
                demo <- raw match
                  ...
                  case _ => Left(ValidationError("Predictor state 'demos' must contain records"))
              yield demos :+ demo
          }
        case None    => Left(ValidationError("Predictor state is missing 'demos'"))
        case Some(_) => Left(ValidationError("Predictor state 'demos' must be a sequence"))
    ...
    def readConfig: Either[DspyError, DynamicValue.Record] =
      DynamicValues.recordGet(state, "config") match
        ...
        case None                              => Left(ValidationError("Predictor state is missing 'config'"))
        case Some(_)                           => Left(ValidationError("Predictor state 'config' must be a record"))
