...
import dspy4s.algebra.{Monoid, MonoidAction}
...
/** `Mode`: a NON-LEARNABLE middleware over a program (Algebra 2's `mode`; see
  ...
  *      [[dspy4s.algebra.Monoid]] trait and hold up to '''extensional equality''' of the wrapped transform (`m1 ≡ m2`
  *      iff `∀ c. m1.transform(c) == m2.transform(c)`), NOT the case class's structural `==` — `++` allocates a fresh
  *      closure each time, so `==` (reference equality on the function field) would reject even `Mode.id ++ m ≡ m`.
  *      `ModeLawSuite` executes the trait's laws through the instance under that extensional equality. Non-commutative,
  *      as an endomorphism monoid is (last write wins: `temperature(0.5)
...
object Mode:
  ...
  /** `Mode` as the endomorphism monoid on `Controls`: `combine` = `++` (compose the control transforms), `empty` =
    * [[Mode.id]]. The associativity / identity laws come from [[dspy4s.algebra.Monoid]] and are executed by
