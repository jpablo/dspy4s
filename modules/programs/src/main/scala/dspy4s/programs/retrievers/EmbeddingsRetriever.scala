...
/** In-memory embedding-similarity retriever over a passage corpus (a port of `dspy.retrievers.Embeddings`, PORT_GAPS
  * G-10): the corpus is embedded once at construction (L2-normalized when `normalize`, the upstream default — cosine
  * similarity); a query is embedded, scored against every passage, and the top `k` are returned with their corpus
  * indices and scores, best first.
  *
  * ==Deltas from Python==
  *   - '''Brute force only.''' Upstream builds a FAISS IVFPQ index for corpora ≥ 20k passages; there is no JVM FAISS
  *     without a native dependency, and below that threshold upstream itself brute-forces — which this port does at any
  *     size (document-scale corpora belong in a real vector store anyway).
