...
object ProgramRunner:
        ...
        case observed: Node.Observe[?, ?] =>
          if !options.traceEnabled then
            ...
            for
              ...
              _      <- append(journal, ProgramEvent.Started(callId, parentCallId, observed.name, encoded))
              result <- evaluate(
                          ...
                          error => append(journal, ProgramEvent.Failed(callId, parentCallId, observed.name, error)),
                          prediction =>
                            append(
                              journal,
  ...
  private def executePredict(
    ...
    val start = callId.fold[UIO[Unit]](ZIO.unit)(id =>
      append(journal, ProgramEvent.Started(id, parentCallId, spec.name, encodedInput))
    )
    val emit = (chunk: PredictionChunk) =>
      callId.fold[UIO[Unit]](ZIO.unit)(id =>
        append(journal, ProgramEvent.OutputChunk(id, parentCallId, spec.name, chunk))
      )
    start *>
      ZIO
        ...
        .tapBoth(
          error =>
            callId.fold[UIO[Unit]](ZIO.unit)(id =>
              append(journal, ProgramEvent.Failed(id, parentCallId, spec.name, error))
            ),
          prediction =>
            callId.fold[UIO[Unit]](ZIO.unit)(id =>
              append(journal, ProgramEvent.Completed(id, parentCallId, spec.name, prediction.raw.values))
            )
        )
