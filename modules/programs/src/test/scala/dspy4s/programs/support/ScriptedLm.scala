...
/** Shared sequential-script LM fake: the i-th `call` returns `responses(i)`; past the end of the script it
  * returns empty text. `calls` exposes how many calls were made. One fixture instead of a per-suite copy so a
  * contract change to [[LanguageModel]] (or a fake-behavior fix) lands in exactly one place. */
