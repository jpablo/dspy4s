...
object DecodeFixtures:
  ...
  /** Install a scripted LM (returning `completion`) plus `adapter` as the active context and run `body` — which invokes
    * whatever program you're testing (`Predict`, `ChainOfThought`, `ReAct`, …). Returns the body's result. This is the
    * program-agnostic core: it doesn't constrain the output type, so it works for `ChainOfThought`'s augmented
    * named-tuple output as readily as a plain `Predict`.
    */
  ...
  /** Run `body` (which invokes some program) under a capturing LM + `adapter`, discard its result, and return the
    * concatenated text of every message the program sent to the LM. Use it to assert *what a program asks the model* —
    * e.g. that enum values or a nested output schema reached the prompt — with no live LM.
    */
  ...
  /** Codec self-consistency for an output type: encode a sample via the signature's output Shape, then decode it back.
    * Proves the dspy4s codec round-trips the type (catches "this type doesn't survive the codec").
    */
