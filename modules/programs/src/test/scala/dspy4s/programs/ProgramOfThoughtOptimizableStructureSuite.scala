...
class ProgramOfThoughtOptimizableStructureSuite extends FunSuite:
  ...
  test("OptimizableStructure exposes generator, regenerator, and answerer in stable named order") {
    val pot = program
    val P   = summon[OptimizableStructure[ProgramOfThought[ProgramOfThoughtStateInput, ProgramOfThoughtStateOutput]]]
  ...
  test("OptimizableStructure replacement obeys Get-Put, read-after-write, and the metadata frame") {
    val pot      = program
    val P        = summon[OptimizableStructure[ProgramOfThought[ProgramOfThoughtStateInput, ProgramOfThoughtStateOutput]]]
