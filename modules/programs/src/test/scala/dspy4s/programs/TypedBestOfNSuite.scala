...
class TypedBestOfNSuite extends FunSuite:
  ...
  private final class TypedStub(results: Vector[Either[DspyError, Prediction[Cand]]])
    ...
    override protected val lifecycle: ModuleLifecycle[Q, Cand]                                                      = ModuleLifecycle.typedWithoutInputs
  ...
  /** A scripted adapter that round-trips any layout generically: it renders every input field (so an injected `hint_`
    * shows up in the prompt) and parses each output field from `metadata`. It records the prompt text of the last
    * inner-`Cand` format so the test can assert the hint reached attempt 2's prompt.
    */
  ...
  /** A scripted LM: for an OfferFeedback prompt it returns the canned [[AdviceText]]; for an inner-`Cand` prompt it
    * returns the GOOD answer (score 1.0) iff the prompt already contains the advice hint, else the BAD answer (score
    * 0.2). Tracks how many OfferFeedback calls it served.
    */
  ...
  /** A scripted LM whose OfferFeedback (advice) call FAILS with a `Left`, while inner-`Cand` calls succeed with the BAD
    * answer (score 0.2). Used to prove that an auxiliary advice-generation failure does NOT discard an
