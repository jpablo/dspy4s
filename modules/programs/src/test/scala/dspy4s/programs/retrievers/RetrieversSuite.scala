...
class RetrieversSuite extends FunSuite:
  ...
  test("NeighborCount rejects non-positive literals and validates runtime values") {
    assert(compileErrors("dspy4s.programs.retrievers.NeighborCount(0)").nonEmpty)
    assert(compileErrors("dspy4s.programs.retrievers.NeighborCount(-1)").nonEmpty)
    assertEquals(NeighborCount.either(3).map(_.toInt), Right(3))
  ...
  private def trainset(examples: Example*): NonEmptyTrainset =
    NonEmptyTrainset.applyUnsafe(examples.toVector)

  private def corpus(passages: String*): NonEmptyCorpus =
    NonEmptyCorpus.applyUnsafe(passages.toVector)

  test("non-empty retriever inputs reject empty runtime collections") {
    assert(NonEmptyTrainset.either(Vector.empty).isLeft)
    assert(NonEmptyCorpus.either(Vector.empty).isLeft)
    assertEquals(NonEmptyCorpus.either(Vector("passage")).map(_.toVector), Right(Vector("passage")))
  }
  ...
  test("KNN retrieves the k nearest trainset examples by dot product, best first") {
    ...
    val knn      = KNN.create(k = NeighborCount(2), examples, embedder).toOption.get
  ...
  test("KNN.create fails fast when the embedder fails (eager trainset embedding)") {
    ...
    intercept[IllegalArgumentException] {
      val _ = KNN.create(k = NeighborCount(1), trainset(ex("q1", "a1"), ex("q2", "a2")), failing)
    }
  }
  ...
  test("KNN.retrieve returns Left (not a NoSuchElementException) when the embedder yields no query row") {
    ...
    val knn    = KNN.create(k = NeighborCount(1), trainset(ex("q1", "a1"), ex("q2", "a2")), embedder).toOption.get
