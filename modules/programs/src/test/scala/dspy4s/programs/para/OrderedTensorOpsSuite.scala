...
class OrderedTensorOpsSuite extends FunSuite:
  ...
  private final case class Fail[I, O](label: String) extends Module[TypedCall[I], Prediction[O]]:
  ...
  test("ordered tensor is not bifunctorial for fail-fast programs") {
    ...
    assertEquals(lhs(TypedCall((1, "x"))).left.map(_.message), Left("g1"))
    assertEquals(rhs(TypedCall((1, "x"))).left.map(_.message), Left("f2"))
  }

  test("tensor preserves values for identity programs") {
    val result = C.tensor(C.id[Int], C.id[String]).apply(TypedCall((5, "x"))).map(_.output)
    assertEquals(result, Right((5, "x")))
  }

  test("swap is involutive and copy is cocommutative on values") {
    val swapped = (C.swap[Int, String] >>> C.swap[String, Int]).apply(TypedCall((5, "x"))).map(_.output)
    val copied = (C.copy[Int] >>> C.swap[Int, Int]).apply(TypedCall(7)).map(_.output)
  ...
  test("discard equality on values hides an observable effect") {
    val f = new Counting
    val lhs = (f >>> C.discard[String]).apply(TypedCall(5)).map(_.output)
    val callsAfterLeft = f.calls.get()
    val rhs = C.discard[Int].apply(TypedCall(5)).map(_.output)
  ...
  test("copy commutes with deterministic programs but not effect-observing programs") {
    ...
    val deterministicLeft = (deterministic >>> C.copy[String]).apply(TypedCall(5)).map(_.output)
    val deterministicRight = (C.copy[Int] >>> C.tensor(deterministic, deterministic)).apply(TypedCall(5)).map(_.output)
    assertEquals(deterministicLeft, deterministicRight)

    val effectful = new Counting
    val effectfulLeft = (effectful >>> C.copy[String]).apply(TypedCall(5)).map(_.output).toOption.get
    val effectfulRight = (C.copy[Int] >>> C.tensor(effectful, effectful)).apply(TypedCall(5)).map(_.output).toOption.get
