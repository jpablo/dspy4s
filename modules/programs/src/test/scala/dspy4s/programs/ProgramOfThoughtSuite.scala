...
class ProgramOfThoughtSuite extends FunSuite:
  ...
  /** Adapter that picks output values from the LM's text by simple convention used only for these scaffold tests:
    *
    *   - signature has `generated_code` output → text is the code, we wrap in fences to mirror what a real LM would do
