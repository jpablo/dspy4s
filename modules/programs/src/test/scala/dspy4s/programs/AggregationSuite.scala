...
/** Direct dspy4s port of Python DSPy's `tests/predict/test_aggregation.py`. Each test mirrors a Python
  * `test_majority_*` case.
  */
class AggregationSuite extends FunSuite:
  ...
  test("majority with Completions picks the most common answer") {
    ...
    val result =
      Aggregation.majorityOf(completions, field = None, normalize = Aggregation.defaultNormalize).toOption.get
    assertEquals(lookup(result.values, "answer"), Some("2": Any))
  }
