...
/** Demonstrates the offline decode-test harness on a representative structured signature. These tests are
  * deterministic, need no API key, and exercise the same list / record / option / lenient-number decode paths that
  * historically broke. Reuses the `TagInput` / `TagItem` / `TagOutput` fixtures from `StructuredOutputSuite` (same
  * package).
  */
