...
class ChainOfThoughtSuite extends FunSuite:
  ...
  /** Adapter that produces both `reasoning` and a set of base output fields. The base fields are configurable via
  ...
  /** Adapter that emits the base fields but FORGETS to emit `reasoning`. Used to verify ChainOfThought surfaces a
    * missing-reasoning error rather than silently dropping the field.
    */
  private class NoReasoningAdapter(baseValues: Map[String, Any]) extends Adapter:
    ...
    override def parse(layout: SignatureLayout, output: LmOutput)(using
        ...
        : Either[DspyError, ParsedOutput] =
      Right(ParsedOutput(values =
        DynamicValues.recordFromEntries(baseValues.toSeq.map((k, v) => k -> DynamicValues.fromAny(v)))
      ))
  ...
  test("ChainOfThought preserves the RawPrediction (including lmUsage)") {
    val sig     = Signature.of[TcotSummarizeSpec]
    val adapter = new ScriptedAdapter(
