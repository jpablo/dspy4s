...
class TransformCombinatorSuite extends FunSuite:
  ...
  private def params[P](program: P)(using predictors: Predictors[P]): Vector[DynamicPredict] =
  ...
  test("mapOutput obeys identity/composition on output and preserves raw prediction evidence") {
    ...
    assertEquals(params(composed), Vector(base.predict))
  }
  ...
  test("dimap is observationally equivalent to contramapInput followed by mapOutput") {
    ...
    assertEquals(params(direct), Vector(base.predict))
  }
  ...
  test("split is ordered independent-input pairing and fails before running the second leg") {
    ...
    assertEquals(params(Compose.split(first, second)), Vector(first.predict, second.predict))
    assertEquals(params(first.split(second)), Vector(first.predict, second.predict))
  }
