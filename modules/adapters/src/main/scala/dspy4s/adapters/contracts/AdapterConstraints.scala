...
/** Renders a consolidated prose constraints block from the output fields that carry
  ...
  * `get_field_description_string` analog; XMLAdapter/JSONAdapter have no such block, so they append THIS block to their
  * system instruction instead (G-9 follow-up). Shared so both render constraints identically.
  */
object AdapterConstraints:

  /** A `Field constraints:` block listing each constrained output field and its constraint strings, or `None` when no
    * output field has constraints (so the prompt is unchanged for unconstrained signatures).
    */
