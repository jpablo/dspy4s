...
/** The narrow public seam onto the otherwise-`private[typed]` [[ZioSchemaCodec]] bridge, for callers outside the typed
  * module that need to move a single `Schema`-typed value across the `DynamicValue` spine (notably
...
object SchemaInterop:
  ...
  /** The wire [[TypeRef]] for a value type, derived from its `Schema` — the same mapping the signature layer applies to
    * each field, exposed one type at a time for argument-schema reporting.
    */
