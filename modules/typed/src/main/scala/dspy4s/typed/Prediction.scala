...
/** A prediction whose raw field values have been decoded into a semantic output
  * value `O`. Constructed only after every required output field
  ...
  *
  * The schema-uninterpreted `RawPrediction` remains available via `raw` so callers can still
...
object Prediction:
  ...
  /** Decodes a `RawPrediction` against an output `Shape`, lifting decode
    * errors into the `Either` channel.
    *
    * Reads the field values directly from `raw.values`. Multi-completion decoding — picking
