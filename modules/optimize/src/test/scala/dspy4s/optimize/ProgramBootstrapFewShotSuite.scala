...
final class ProgramBootstrapFewShotSuite extends FunSuite:
  ...
  test("bootstrap few-shot uses effectful record execution and stable parameter IDs") {
    ...
    val report = run(ProgramBootstrapFewShot(
      program,
      trainset,
      config = ProgramBootstrapFewShotConfig(maxBootstrappedDemos = DemoCount(2))
    ))
