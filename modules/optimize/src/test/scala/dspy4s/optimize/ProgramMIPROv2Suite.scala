...
final class ProgramMIPROv2Suite extends FunSuite:
  ...
  private val config = ProgramMIPROv2Config(
    metric = metric,
  ...
  private def run(
      proposer: ProgramWithEnv[ProgramMIPROv2.ProposalInput, ProgramMIPROv2.Proposal, Any]
  ) = Unsafe.unsafe { implicit unsafe =>
    Runtime.default.unsafe
      .run(
        ProgramMIPROv2(student, dataset, proposer, valset = Some(dataset), config = config)
  ...
  test("program MIPROv2 selects a proposed instruction by stable parameter ID") {
    val inputs   = ArrayBuffer.empty[ProgramMIPROv2.ProposalInput]
    val proposer = Program.lift[ProgramMIPROv2.ProposalInput, ProgramMIPROv2.Proposal] { input =>
      inputs += input
      ProgramMIPROv2.Proposal("good instruction")
    }
  ...
  test("program MIPROv2 records proposal failures and keeps the baseline") {
    val proposer = Program.liftEither[ProgramMIPROv2.ProposalInput, ProgramMIPROv2.Proposal](_ =>
  ...
  test("program MIPROv2 creates the same trial plans for the same seed") {
    val proposer = Program.lift[ProgramMIPROv2.ProposalInput, ProgramMIPROv2.Proposal](_ =>
      ProgramMIPROv2.Proposal("good instruction")
    )
