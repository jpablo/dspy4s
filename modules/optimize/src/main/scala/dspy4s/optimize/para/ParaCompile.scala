...
/** A packaged [[Program]] as the optimizer entry point.
  *
  * `COPRO[P]` needs `OptimizableStructure[P]` and `LegacyProgramRunner[P]`. The packaged entry point therefore retains
...
object ParaCompile:
  extension [I, O, N <: Int](program: Program[I, O, N])
    /** Run COPRO over a packaged program while preserving its static parameter arity. The usual `LegacyProgramRunner`
    ...
    def copro(
        ...
        RuntimeContext,
        LegacyProgramRunner[Program[I, O, N]]
