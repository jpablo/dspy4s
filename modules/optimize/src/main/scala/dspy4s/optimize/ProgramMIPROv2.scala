...
final case class ProgramMIPROv2Config(
...
object ProgramMIPROv2:
  ...
  private def generateDemoCandidates[I, O](
    ...
    ZIO.foldLeft(0 until config.numCandidates)(DemoState(Vector(zeroShot), 0)) { (state, candidate) =>
      ProgramBootstrapFewShot(
        ...
        case Right(report) =>
          val assignment = report.bestProgram.program.parameters.all.map(binding => binding.id -> binding.value.demos).toMap
  ...
  private def evaluateTrials[I, O](
    ...
    ProgramEvaluate(student, evalset, config.metric, config.evaluation).flatMap { baselineEvaluation =>
      ...
      ZIO.foldLeft(plans.zipWithIndex)(Vector(baseline)) { case (candidates, (plan, trial)) =>
        ZIO.fromEither(applyTrial(student, demos.assignments(plan.demoIndex), instructions.values, plan)).flatMap {
          program =>
            ProgramEvaluate(program, evalset, config.metric, config.evaluation).map { evaluation =>
