...
import dspy4s.programs.plan.{PredictionBackend, RecordProgram}
import zio.ZIO

final case class ProgramBootstrapRandomSearchConfig(
...
object ProgramBootstrapRandomSearch:
    ...
    def loop(
      ...
      remaining match
        ...
        case (seed, program) :: tail =>
          ProgramEvaluate(program, valset, config.metric, config.evaluation).flatMap { evaluation =>
