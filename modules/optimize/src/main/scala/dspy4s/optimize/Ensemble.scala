...
/** Port of Python DSPy's `dspy.teleprompt.ensemble.Ensemble`.
  ...
  * build on the untyped spine: members are `DynamicModule`s, the reducer folds their `DynamicPrediction.values` rows,
  * and the result is itself a `DynamicModule` (so an ensemble can nest / be used anywhere a program is). This matches
  * the "prefer the untyped spine for genuinely-generic edges" guidance.
  *
