...
import dspy4s.programs.plan.{PredictionBackend, ProgramRunner, RecordProgram, RunOptions}
import zio.ZIO

final case class ProgramBootstrapFewShotConfig(
    metric              : Option[ProgramMetric] = None,
...
object ProgramBootstrapFewShot:
  ...
  def apply[I, O](
      ...
      config  : ProgramBootstrapFewShotConfig = ProgramBootstrapFewShotConfig()
  ...
  private def tryRounds[I, O](
      ...
      example : Example,
      config  : ProgramBootstrapFewShotConfig,
      errorsAtStart: Int
            ...
            execution.outcome match
              ...
              case Right(prediction) =>
                config.metric match
                  ...
                  case Some(metric) =>
                    metric.score(example, prediction.raw, execution.events).either.flatMap {
                      case Left(error)  => continueAfterError(round, errors, error)
                      case Right(score) =>
