...
/** Live integration tests that hit a real OpenAI-compatible endpoint. These are dspy4s ports of the
  * `@pytest.mark.llm_call`-marked tests from Python DSPy's `tests/streaming/test_streaming.py`. They exercise the full
  * streaming pipeline — adapter formatting, real SSE chunks, per-field routing, listener filtering — against an actual
  * model.
  *
  ...
  *
  * Without both, every test calls `assume(...)` and reports as skipped, so `sbt test` passes without credentials.
  *
  * Cost: each test pins `max_tokens` low and `temperature=0`, so a full run is roughly $0.001 against `gpt-4o-mini`.
  */
class StreamingLiveSuite extends FunSuite:
  ...
  /** Shared MyProgram scaffold used by both the Chat and JSON adapter ports. Mirrors Python's `class
    * MyProgram(dspy.Module)` in tests/streaming/test_streaming.py.
    */
  private def buildComposite(): DynamicModule =
  ...
  /** Direct dspy4s port of Python DSPy's `tests/streaming/test_streaming.py::test_stream_listener_chat_adapter`.
    ...
    *
    * Streams with two listeners (one per output field) and asserts that the first chunk comes from `predict1`/`answer`
    * and the second from `predict2`/`judgement`. The Python test uses the same first/second assertion pattern with a
    * tolerance for the trailing chunk.
    */
  ...
  /** Direct dspy4s port of Python DSPy's `tests/streaming/test_streaming.py::test_sync_streaming`.
    *
    * In Python, `streamify` returns an async iterator by default and this test pins `async_streaming=False` to exercise
    * the sync opt-out. dspy4s's `Streamify` has no async variant — it returns a [[ClosableIterator]] driven by a
    * producer thread, which is exactly the "sync streaming" mode Python is testing. The test therefore looks identical
    * to the chat adapter parity test above; both are kept so the Python parity matrix stays 1:1 and so a future async
    * variant has an obvious place to add its mirror test.
    */
  ...
  /** Direct dspy4s port of Python DSPy's `tests/streaming/test_streaming.py::test_stream_listener_json_adapter`.
    *
    * Same MyProgram, same listeners, but with [[JSONAdapter]] instead of [[ChatAdapter]]. Asserts the first chunk comes
    * from `predict1`/`answer` and the last from `predict2`/`judgement` — the JSON adapter has no completion-marker
    * quirk so Python uses `[-1]` directly here.
    */
