...
/** Carries the active [[RuntimeContext]] across thread boundaries.
  *
  * '''Why this is needed.''' [[RuntimeEnvironment]] keeps the active context -- the configured LM / adapter / callbacks
  * plus the accumulated trace, history, and call stack -- in a `ThreadLocal`. Thread-locals do not follow work that
  * hops to another thread, so when a program runs on a pool thread (a `Future`, a parallel executor worker, a streaming
...
object ContextPropagation:
  ...
  /** A module-owned piece of thread-local state that must accompany the [[RuntimeContext]] whenever work hops threads.
    * Modules register one via [[registerCarrier]]; every capture point then snapshots it on the submitting thread and
    * re-installs it on the worker for the duration of the task.
    */
  trait Carrier:
  ...
  /** Register a [[Carrier]] for the lifetime of the process. Idempotence is the caller's concern (register from an
    * `object` initializer so it runs once).
    */
  def registerCarrier(carrier: Carrier): Unit =
    val _ = carriers.add(carrier)

  /** Everything that must travel to a worker thread: the [[RuntimeContext]] plus a snapshot of every registered
    * carrier.
    */
  ...
  /** Run `body` on a [[wrapExecutionContext]]-wrapped `base`, so the resulting `Future` executes under the context
    * captured at the call site rather than the worker thread's empty default.
    */
  def future[A](
      body: => A
  ...
  /** Run `body` asynchronously under the captured services/config/scope and a fresh output delta. Unlike [[future]],
    * the trace and history produced on the worker are returned explicitly in [[Executed]] instead of disappearing when
    * the worker's thread-local context is restored. The submitting thread is never mutated; callers may deliberately
    * replay the returned delta with [[RuntimeEnvironment.propagate]].
    */
